...
**Wire goldens (ruling R236).** Every binary format has a committed golden
fixture generated by the engine and decoded in the app's tests
(`app/src/ipc/golden/`, `core/src/wire_golden.rs`, `idl-rs docs wire`) — a
cross-language check that the Rust encoder and the TS decoder still agree on
every field, not just that each side's own unit tests still pass.
