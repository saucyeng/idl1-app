...
import { buildScreenPalette } from "../model/report/screenPalette";
 ...
 * 2. **Which palette.** The app's live `--chart-N` tokens
 *    (`model/report/screenPalette.ts`), never `printPalette.ts`'s
 *    black-on-white, so a lap is the same colour here as in the notebook.
 ...
 */
export default function PaperView({ document, onSelectCell }: PaperViewProps) {
  ...
  const palette = useMemo(() => buildScreenPalette(documentVars()), []);
  ...
  return (
    <article className="paper-view" onClick={handleTap}>
