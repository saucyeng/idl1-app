...
- Task 12 — `489247e` cursor: nearest-sample readout, clamped inside span / `null` outside (R31), one algorithm shared with `SessionHandle` (C3 §3.7); `039df03` R39 (pre-existing `gps_channel_values` amended to also go `null` past a channel's span, matching `cursor_readout`).
