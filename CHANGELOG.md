...
- **Data tab: filter model, facets and filter rail.** `DataFilters`/`filtersReducer` (Date, Track, Bike, Rider, Tag, Venue, Lap time, Source facets, AND across categories / OR within a facet, `""` = "(none)") plus `matchesFilters`/`facetCounts` and the `FilterRail`/`ActiveChips` UI, ported from idl0's `data_filters_provider.dart`/`filter_rail.dart`. Has-gates and has-GPS facets are dropped outright for wave 2 (R53 Data Q2) — neither is derivable from a `SessionSummary`. The Track facet lists its options from `list_tracks` but cannot yet filter sessions (no session→track linkage exists without lap indexing, R53 Data Q4); Lap-time filters key off `duration_ms` for the same reason. See `docs/IDL0_SPEC.md` §24.4.
